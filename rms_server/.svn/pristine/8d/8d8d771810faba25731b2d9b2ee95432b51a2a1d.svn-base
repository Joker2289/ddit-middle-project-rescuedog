...
public class AdoptBoardAdminDaoImpl implements IAdoptBoardAdminDao {
	private static AdoptBoardAdminDaoImpl dao;
	private SqlMapClient smc;

	public AdoptBoardAdminDaoImpl() {
		smc = XmlConnection.getConnection();
	}

	public static AdoptBoardAdminDaoImpl getInstance() {
		if (dao == null) {
			dao = new AdoptBoardAdminDaoImpl();
		}
		return dao;
	}
	...
	@Override
	public int deleterescuedog(RescuedogVO vo) {
		int cnt = 0;
		try {
			cnt = smc.delete("rescuedog.deleterescuedog", vo);
