...
public class Protected_Add implements Initializable {
 ...
 Protected_boardVO ppvo = new Protected_boardVO();
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			CustomVO ctemp =Main.fis.getSearchCustom(cvo);
			Tel_Fb.setText(ctemp.getCustom_tel());
...
//					vo.setName(Name_Fb.getText());
					...
					try {
						loadPane=FXMLLoader.load(IProtectedBoardUserService.class.getResource("protectedBoard_View.fxml"));
	...
	private void fileAdd() {
		...
		Protected_boardVO vo = new Protected_boardVO();
		try {
			for (int j = 0; j < Main.pbus.getAllBoardList().size(); j++) {
				Main.pbus.getAllBoardList().get(j).getBoard_num();
				ppvo.setBoard_num(Main.pbus.getAllBoardList().get(j).getBoard_num());
...
//			File downloadFile = new File("\\\\Sem-pc\\공유폴더\\Rescuedog\\Protected_Board\\"+ProtectedBoard_Cotroller.bc_num+fileName);
//			File downloadFile = new File("\\\\Sem-pc\\공유폴더\\Rescuedog\\Protected_Board\\"+login_controller.log.getMem_id()+","+ppvo.getBoard_num()+ZfileName);
...
//			File downloadFile = new File("C:\\Users\\shrtm\\OneDrive\\바탕 화면\\testpath"+fileName);
			...
			System.out.println(ppvo.getBoard_num()+"@@@@@@@@@@@@@@@@");
