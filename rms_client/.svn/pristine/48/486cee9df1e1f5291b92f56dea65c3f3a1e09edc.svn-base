...
public class ShareUController implements Initializable {
	...
	private void tableSet() {
		...
		try {
			lists = (ArrayList<Board_detailVO>) Main.sib_u.getBoard_detailAll(); // 목록을 전체 가져옴
			if (lists.isEmpty()) {
				return;
			}
	...
	private void findMember() {
		...
		if (Table_search_cb.getSelectionModel().isEmpty()) {
			...
			allBoardData = FXCollections.observableArrayList();
			try {
				lists = (ArrayList<Board_detailVO>) Main.sib_u.getSearchBoard_detail(vo);
				for (int i = 0; i < lists.size(); i++) {
					allBoardData.add(lists.get(i)); // 목록 전체 길이 구하기
				}
				tableSet();
			...
			case "제목":
				try {
					...
					ArrayList<Board_detailVO> searchList = (ArrayList<Board_detailVO>) Main.sib_u
	...
	private void gotoshareAdd() {
		if (lists.isEmpty()) {
			...
			try {
				...
				Share_addUController.board_num = tempBns.get(0).getBc_idx();
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
			Share_addUController.bc_num = share_table.getItems().get(0).getBc_num();
		}
	...
	private void gotoshareDetail() {
		Share_detailUController.vo = share_table.getSelectionModel().getSelectedItem();

		Board_detailVO tempVO = Share_detailUController.vo;
		int cntPlus = Integer.parseInt(tempVO.getBoard_cnt()) + 1;
		tempVO.setBoard_cnt(cntPlus + "");
		Share_detailUController.vo = tempVO;

		try {
			Main.sib_u.updateBoard_detail(tempVO);
