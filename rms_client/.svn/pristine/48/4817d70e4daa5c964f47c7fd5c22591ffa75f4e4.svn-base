...
import kr.or.ddit.rms.user.review_board.Review_board_detailUController;
import kr.or.ddit.rms.vo.Board_classVO;
import kr.or.ddit.rms.vo.Board_detailVO;

public class Free_boardTbUController implements Initializable {
	...
	@FXML
	TableView<Board_detailVO> Freeboard_table;
	@FXML
	TableColumn<Board_detailVO, String> Table_freeboard_col1;
	@FXML
	TableColumn<Board_detailVO, String> Table_freeboard_col2;
	@FXML
	TableColumn<Board_detailVO, String> Table_freeboard_col3;
	@FXML
	TableColumn<Board_detailVO, String> Table_freeboard_col4;
	@FXML
	TableColumn<Board_detailVO, String> Table_freeboard_col5;
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		Freeboard_table.setOnMouseClicked(e -> {
			if (e.getClickCount() == 2) {
				gotoFreeBoardDetail();
			}
		});
		Table_add_btn.setOnAction(e -> gotoFreeBoardAdd());
	...
	private void tableSet() {
		...
		try {
			lists = (ArrayList<Board_detailVO>) Main.fb_u.getBoard_detailAll(); // 목록을 전체 가져옴
			if (lists.isEmpty()) {
				return;
			}
		...
		Table_freeboard_col1.setCellValueFactory(new PropertyValueFactory<>("board_num"));
		Table_freeboard_col2.setCellValueFactory(new PropertyValueFactory<>("title"));
		Table_freeboard_col3.setCellValueFactory(new PropertyValueFactory<>("custom_id"));
		Table_freeboard_col4.setCellValueFactory(new PropertyValueFactory<>("upd_date"));
		Table_freeboard_col5.setCellValueFactory(new PropertyValueFactory<>("board_cnt")); // 컬럼 설정
	...
	private void setTable() {
		...
		Freeboard_table.setItems(allBoardData);
	...
	private void findMember() {
		...
		if (Table_search_cb.getSelectionModel().isEmpty()) {
			...
			allBoardData = FXCollections.observableArrayList();
			try {
				lists = (ArrayList<Board_detailVO>) Main.fb_u.getSearchBoard_detail(vo);
				for (int i = 0; i < lists.size(); i++) {
					allBoardData.add(lists.get(i)); // 목록 전체 길이 구하기
				}
				tableSet();
			...
			case "제목":
				try {
					...
					ArrayList<Board_detailVO> searchList = (ArrayList<Board_detailVO>) Main.fb_u
							.getFree_detailTextSearch(tempVO);
	...
	private void gotoFreeBoardAdd() {
		if (lists.isEmpty()) {
			Free_board_addUController.bc_num = "1";
		} else {
			try {
				lists = (ArrayList<Board_detailVO>) Main.fb_u.getBoard_detailAll(); // 목록을 전체 가져옴
				...
				Free_board_addUController.board_num = tempBns.get(0).getBc_idx();
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
			Free_board_addUController.bc_num = Freeboard_table.getItems().get(0).getBc_num();
		}
		ChangePane cp = Main_page_controller.fxmlLoad.getController();
		cp.clearing();
		ChangePane.changePane(getClass().getResource("free_board_add.fxml"));
		cp.add();

	}

	private void gotoFreeBoardDetail() {
		Free_board_detailUController.vo = Freeboard_table.getSelectionModel().getSelectedItem();

		Board_detailVO tempVO = Free_board_detailUController.vo;
		int cntPlus = Integer.parseInt(tempVO.getBoard_cnt()) + 1;
		tempVO.setBoard_cnt(cntPlus + "");
		Free_board_detailUController.vo = tempVO;

		try {
			Main.fb_u.updateBoard_detail(tempVO);
		} catch (RemoteException e2) {
			e2.printStackTrace();
		}
		ChangePane cp = Main_page_controller.fxmlLoad.getController();
		cp.clearing();
		ChangePane.changePane(getClass().getResource("free_board_detail.fxml"));
		cp.add();
	}
	...
	private Node createPage(int pageIndex) {
		...
		Freeboard_table.setItems(getTableViewData(from, to));

		return Freeboard_table;
	}
