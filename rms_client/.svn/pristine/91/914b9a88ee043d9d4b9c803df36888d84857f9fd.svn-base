...
public class Share_detailController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		share_comment_name_textfield.setText(Login_controller.log.getMem_id());
		...
		share_del_btn.setOnAction(e -> {
			...
			if (delcheck) {
				try {
					Main.sib_a.deleteBoard_detail(Share_detailController.vo);
					ChangePane cp = Main_page_controller.fxmlLoad.getController();
					cp.clearing();
					ChangePane.changePane(getClass().getResource("share_table.fxml"));
					cp.add();
	...
	private void addCom() {
		if (share_comment_add_ta.getText().length() == 0 || share_comment_add_ta.getText().length() < 5) {
			...
			try {
				Main.sib_a.updateBoard_detail(vo);
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
			temp.setBc_num(vo.getBc_num());
	...
	private void updCheck(JFXTextField title, JFXTextArea content, Stage writePage) {
		if (title.getText().isEmpty()) {
			...
			try {
				int cnt = Main.sib_a.updateBoard_detail(uvo);
				if (cnt > 0) {
					alertInfo("수정 완료");
					vo = uvo;
					share_title_tf.setText(vo.getTitle());
