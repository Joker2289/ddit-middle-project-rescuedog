...
public class ProtectedBoard_Cotroller implements Initializable {
	...
	@FXML Label top_lb2;
//	@FXML FontAwesomeIcon camera_icon;
	...
	public void initialize(URL location, ResourceBundle resources) {
		try {
			viewList = Main.pbss.getAllBoardList();
			setVbox(viewList);
...
String[] search_item =  {"작성자명", "특징", "견종","발견위치"};
		...
		ProtectedSerch_btn.setOnAction(e->{
			...
			switch(Search_Comb.getValue().toString()) {
				case "유기견 이름":
					...
					try {
						List<Protected_boardVO> searchList = Main.pbss.getprotectedTextSearch(vo);
				...
				case "작성자명":
					...
					try {
						List<Protected_boardVO> searchList = Main.pbss.getprotectedTextSearch(Membervo);
						if (searchList.size() == 0) {
							Protected_Pane.getChildren().clear();
							System.out.println("검색없음");
							inviserble_lb.setText("검색된 결과가 없습니다.");
				...
				case "특징":
					...
					try {
						List<Protected_boardVO> searchList = Main.pbss.getprotectedTextSearch(Characteristicvo);
						if (searchList.size() == 0) {
							Protected_Pane.getChildren().clear();
							System.out.println("검색없음");
							inviserble_lb.setText("검색된 결과가 없습니다.");
				...
				case "견종":
					...
					try {
						List<Protected_boardVO> searchList = Main.pbss.getprotectedTextSearch(Kindvo);
						if (searchList.size() == 0) {
							Protected_Pane.getChildren().clear();
							System.out.println("검색없음");
							inviserble_lb.setText("검색된 결과가 없습니다.");
				...
				case "발견위치":
					...
					try {
						List<Protected_boardVO> searchList = Main.pbss.getprotectedTextSearch(Findvo);
						if (searchList.size() == 0) {
							Protected_Pane.getChildren().clear();
							System.out.println("검색없음");
							inviserble_lb.setText("검색된 결과가 없습니다.");
		...
		// 검색 버튼 이벤트
//		ProtectedSerch_btn.setOnAction(new EventHandler<ActionEvent>() {
//
//			@Override
//			public void handle(ActionEvent event) {
//				// TODO Auto-generated method stub
//
//				// System.out.println(viewList.get(0).getKind());
//				System.out.println("시작" + ProtectedSerch_fd.getText());
//				Protected_boardVO vo = new Protected_boardVO();
////			vo.setMem_id(ProtectedSerch_fd.getText());
//				vo.setMem_id(ProtectedSerch_fd.getText());
//				vo.setKind(ProtectedSerch_fd.getText());
//				vo.setFind_loc(ProtectedSerch_fd.getText());
//				try {
//					List<Protected_boardVO> searchList = Main.pbss.getprotectedTextSearch(vo);
//					if (searchList.size() == 0) {
//						Protected_Pane.getChildren().clear();
...
//		            new FileInputStream("\\\\Sem-pc\\공유폴더\\Rescuedog\\Protected_Board\\"+bc_num+viewList.get(i).getImg()));
			...
			kind_lb.setText("     견종  :" + pvo.getKind());
	...
	private void viewEvent(MouseEvent e, Image protected_image, Protected_boardVO protected_boardVO) {
		...
		JFXTextField Path_Fb = (JFXTextField) loadPane.lookup("#Path_Fb");
...
//		Image nmage = new Image("file:///C:/Users/shrtm/OneDrive/바탕 화면/testpath"+imgString[0]);
		...
		if (!writer_lb.getText().equals(Login_controller.log_s.getShel_id())) {
		...
		Update_Btn.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				try {
					loadPane = FXMLLoader
							.load(IProtectedBoardShelterService.class.getResource("protectedBoard_View.fxml"));
		...
		Delete_Btn.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				deleteVO.setMem_id(Login_controller.log_s.getShel_id());

				try {
					Main.pbss.deleteBoard(deleteVO);
...
//			File downloadFile = new File("D:\\A_TeachingMaterial\\4.MiddleProject\\workspace\\rms_client\\src\\kr\\or\\ddit\\rms\\user\\protected_animal_board\\protected_img"+fileName);
