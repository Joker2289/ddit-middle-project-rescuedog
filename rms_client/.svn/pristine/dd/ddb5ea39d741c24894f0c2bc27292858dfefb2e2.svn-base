...
public class Adopt_viewController implements Initializable{
 
	@FXML AnchorPane Adopt_Main_Pane;
	@FXML JFXComboBox Search_Comb;
	@FXML JFXTextField Search_Txt;
	...
	private void show_rdList(List<RescuedogVO> list) {
		...
		try {
			...
			//List<RescuedogVO> rdlist = Main.ab_s.getRescuedogAll();
			//list_size = rdlist.size();
			...
			for(int i=0; i<list.size(); i++) {
				...
				view.setAccessibleText(rd_img+"/"+rd_num+"/"+rd_name+"/"+
							rd_kind+"/"+rd_gender+"/"+
							find_date+"/"+ shel_name+"/"+
							rd_info+"/"+rd_check +"/"+ list.get(i).getShel_id());
