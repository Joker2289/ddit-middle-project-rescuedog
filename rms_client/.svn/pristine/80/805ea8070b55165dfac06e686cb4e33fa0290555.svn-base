...
public class SponRecruitController implements Initializable {
	...
	@FXML
	TableView<SponVO> recuruit_table;
	@FXML
	TableColumn<SponVO, String> recuruit_numCol1;
	@FXML
	TableColumn<SponVO, String> recuruit_titleCol2;
	@FXML
	TableColumn<SponVO, String> recuruit_nameCol3;
	@FXML
	TableColumn<SponVO, String> recuruit_sdateCol4;
	@FXML
	TableColumn<SponVO, String> recuruit_edateCol5;
	@FXML
	TableColumn<SponVO, String> recuruit_priceCol6;

	@FXML
	Button back;
	public static SponVO addVO;
	...
	private void addEvent() {
		...
		cp.clearing();
		ChangePane.changePane(getClass().getResource("spon_recruit.fxml"));
		cp.add();
	}


	private void gotorecruitDetail() {
		SponRecruitDetailController.vo = recuruit_table.getSelectionModel().getSelectedItem();
		ChangePane cp = Main_page_controller.fxmlLoad.getController();
		cp.clearing();
		ChangePane.changePane(getClass().getResource("spon_recruitDetail.fxml"));
		cp.add();
	...
	private void searchAll() {
		ShelterVO svo = new ShelterVO();
		try {
			lists = (ArrayList<SponVO>) Main.sr_s.getSponAll();
			if(lists.isEmpty()) {
				return;
			}
			for (int i = 0; i < lists.size(); i++) {
				...
				allBoardData = FXCollections.observableArrayList(lists);
			}
		} catch (RemoteException e) {
			e.printStackTrace();
		}
