...
public class Share_detailUController implements Initializable {
	public static Board_detailVO vo; // 목록화면에서 선택했을 때 선택한 값을 저장하기 위한 vo
	@FXML
	JFXTextField share_title_tf;
	@FXML
	JFXTextField share_id_tf;
	@FXML
	ImageView Layout_logo_note_img;
	@FXML
	ImageView share_content_iv;
	@FXML
	JFXButton report_btn;
	@FXML
	Label share_upd_date_label;
	@FXML
	Label share_cnt_label;
	@FXML
	WebView share_content_wv;
	@FXML
	JFXTextField share_comment_name_textfield;
	@FXML
	JFXTextArea share_comment_add_ta;
	@FXML
	JFXButton share_comment_add_btn;
	@FXML
	JFXButton share_upd_btn;
	@FXML
	JFXButton share_del_btn;
	@FXML
	JFXButton share_gotolist_btn;
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		share_title_tf.setText(vo.getTitle());
		share_id_tf.setText(vo.getCustom_id());
		share_upd_date_label.setText(vo.getUpd_date());
		share_cnt_label.setText(vo.getBoard_cnt());

		contentLoad();
		share_title_tf.setEditable(false);
		share_id_tf.setEditable(false);
		share_content_wv.setDisable(true);
		share_upd_btn.setOnAction(e -> modalupd());
		report_btn.setOnAction(e -> modalReport());
		if (!vo.getCustom_id().equals(Login_controller.log_c.getCustom_id())) {
			share_upd_btn.setDisable(true);
			share_del_btn.setDisable(true);
		}
		share_comment_name_textfield.setText(Login_controller.log_c.getCustom_name());
		share_comment_name_textfield.setDisable(true);
		commentTableSet();
		imgSet();
		share_gotolist_btn.setOnAction(new EventHandler<ActionEvent>() {
		...
		share_del_btn.setOnAction(e -> {
			boolean delcheck = alertConfirm("삭제 하시겠습니까?");
			if (delcheck) {
				try {
					Main.sib_u.deleteBoard_detail(Share_detailUController.vo);
					ChangePane cp = Main_page_controller.fxmlLoad.getController();
					cp.clearing();
					ChangePane.changePane(getClass().getResource("share_table.fxml"));
					cp.add();
	...
	private void contentLoad() {
		WebEngine webEngine = share_content_wv.getEngine();
	...
	private void imgSet() {
		if (vo.getBoard_img() != null) {
			File loadFile = new File("\\\\Sem-pc\\공유폴더\\Rescuedog\\Sharing_info\\" + vo.getBc_num() + ","
			...
			try {
				...
				share_content_iv.setImage(img);
			} catch (IOException e) {
				forRemove_vb.getChildren().remove(share_content_iv);
	...
	private void commentTableSet() {
		...
		share_comment_add_btn.setOnAction(e -> addCom());
	...
	private void addCom() {
		if (share_comment_add_ta.getText().length() == 0 || share_comment_add_ta.getText().length() < 5) {
			...
			try {
				Main.sib_u.updateBoard_detail(vo);
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
			temp.setBc_num(vo.getBc_num());
			temp.setBoard_num(vo.getBoard_num());
			temp.setComment_content(share_comment_add_ta.getText());
			temp.setCustom_id(Login_controller.log_c.getCustom_id());
			try {
				Main.cs.insertComments(temp);
				share_comment_add_ta.clear();
	...
	private void modalReport() {
		...
		idTo.setText(this.share_id_tf.getText());
		...
		ok.setOnAction(e -> {
			...
			if (check) {
				try {
					...
					bvo.setMem_id(this.share_id_tf.getText());
	...
	private void modalupd() {
		...
		Parent parent = null;
		try {
			parent = FXMLLoader.load(getClass().getResource("share_detail_upd.fxml"));
		} catch (IOException e) {
			e.printStackTrace();
		}

		JFXButton cancel = (JFXButton) parent.lookup("#share_cancel_btn");
		JFXButton ok = (JFXButton) parent.lookup("#share_ok_btn");
		JFXTextField title = (JFXTextField) parent.lookup("#share_title_tf");
		JFXTextField id = (JFXTextField) parent.lookup("#share_customId_tf");
		JFXTextArea content = (JFXTextArea) parent.lookup("#share_content_ta");
		title.setText(this.share_title_tf.getText());
		id.setText(this.share_id_tf.getText());
	...
	private void updCheck(JFXTextField title, JFXTextArea content, Stage writePage) {
		if (title.getText().isEmpty()) {
			...
			try {
				int cnt = Main.sib_u.updateBoard_detail(uvo);
				if (cnt > 0) {
					alertInfo("수정 완료");
					vo = uvo;
					share_title_tf.setText(vo.getTitle());
