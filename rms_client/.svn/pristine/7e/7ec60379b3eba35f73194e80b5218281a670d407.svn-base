...
public class Review_board_detailController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		review_comment_name_textfield.setText(Login_controller.log.getMem_id());
		commentTableSet();
		imgSet();
		...
		review_del_btn.setOnAction(e -> {
			...
			if (delcheck) {
				try {
					Main.rb_a.deleteBoard_detail(Review_board_detailController.vo);
					ChangePane cp = Main_page_controller.fxmlLoad.getController();
					cp.clearing();
					ChangePane.changePane(getClass().getResource("review_board_table.fxml"));
					cp.add();
	...
	private void sendNote() {
		...
		send.setOnAction(e -> {
			if (note_send_to_txtfield.getText().isEmpty()) { // 받는 사람이 비어있을때
			...
			} else if (vo.getCustom_id().equals(Login_controller.log.getMem_id())) {
				...
				vo1.setNote_id_from(Login_controller.log.getMem_id()); // 받는 사람에 보내는 사람(로그인한) id 넣기
	...
	private void addCom() {
		if (review_comment_add_ta.getText().length() == 0 || review_comment_add_ta.getText().length() < 5) {
			...
			try {
				Main.rb_a.updateBoard_detail(vo);
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
			...
			temp.setCustom_id(Login_controller.log.getMem_id());
			try {
	...
	private void modalReport() {
		...
		idFrom.setText(Login_controller.log.getMem_id());
	...
	private void updCheck(JFXTextField title, JFXTextArea content, Stage writePage) {
		if (title.getText().isEmpty()) {
			...
			try {
				int cnt = Main.rb_a.updateBoard_detail(uvo);
				if (cnt > 0) {
					alertInfo("수정 완료");
					vo = uvo;
					review_title_tf.setText(vo.getTitle());
