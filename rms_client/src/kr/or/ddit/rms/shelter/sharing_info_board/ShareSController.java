...
import kr.or.ddit.rms.user.review_board.Review_board_addUController;
import kr.or.ddit.rms.user.review_board.Review_board_detailUController;
import kr.or.ddit.rms.user.sharing_info_board.Share_detailUController;
...
public class ShareSController implements Initializable {
	...
	@FXML
	TableView<Board_detailVO> share_table;
	@FXML
	TableColumn<Board_detailVO, String> Table_share_col1;
	@FXML
	TableColumn<Board_detailVO, String> Table_share_col2;
	@FXML
	TableColumn<Board_detailVO, String> Table_share_col3;
	@FXML
	TableColumn<Board_detailVO, String> Table_share_col4;
	@FXML
	TableColumn<Board_detailVO, String> Table_share_col5;
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		share_table.setOnMouseClicked(e -> {
			if (e.getClickCount() == 2) {
				gotoshareDetail();
			}
		});
		Table_add_btn.setOnAction(e -> gotoshareAdd());
	...
	private void tableSet() {
		...
		try {
			lists = (ArrayList<Board_detailVO>) Main.sib_s.getBoard_detailAll(); // 목록을 전체 가져옴
			if (lists.isEmpty()) {
				return;
			}
		...
		Table_share_col1.setCellValueFactory(new PropertyValueFactory<>("board_num"));
		Table_share_col2.setCellValueFactory(new PropertyValueFactory<>("title"));
		Table_share_col3.setCellValueFactory(new PropertyValueFactory<>("custom_id"));
		Table_share_col4.setCellValueFactory(new PropertyValueFactory<>("upd_date"));
		Table_share_col5.setCellValueFactory(new PropertyValueFactory<>("board_cnt")); // 컬럼 설정
	...
	private void setTable() {
		...
		share_table.setItems(allBoardData);
	}
	...
	private void findMember() {
		...
		if (Table_search_cb.getSelectionModel().isEmpty()) {
			...
			allBoardData = FXCollections.observableArrayList();
			try {
				lists = (ArrayList<Board_detailVO>) Main.sib_s.getSearchBoard_detail(vo);
				for (int i = 0; i < lists.size(); i++) {
					allBoardData.add(lists.get(i)); // 목록 전체 길이 구하기
				}
				tableSet();
			...
			case "제목":
				try {
					...
					ArrayList<Board_detailVO> searchList = (ArrayList<Board_detailVO>) Main.sib_s
							.getShare_detailTextSearch(tempVO);
					if (searchList == null) {
						alertInfo("정보가 없습니다.");
					} else {
						allBoardData.clear();
	...
	private void gotoshareAdd() {
		if (lists.isEmpty()) {
			Share_addSController.bc_num = "2";
		} else {
			try {
				lists = (ArrayList<Board_detailVO>) Main.sib_s.getBoard_detailAll(); // 목록을 전체 가져옴
				...
				Share_addSController.board_num = tempBns.get(0).getBc_idx();
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
			Share_addSController.bc_num = share_table.getItems().get(0).getBc_num();
		}
		ChangePane cp = Main_page_controller.fxmlLoad.getController();
		cp.clearing();
		ChangePane.changePane(getClass().getResource("share_add.fxml"));
		cp.add();

	}

	private void gotoshareDetail() {
		Share_detailSController.vo = share_table.getSelectionModel().getSelectedItem();

		Board_detailVO tempVO = Share_detailSController.vo;
		int cntPlus = Integer.parseInt(tempVO.getBoard_cnt()) + 1;
		tempVO.setBoard_cnt(cntPlus + "");
		Share_detailSController.vo = tempVO;

		try {
			Main.sib_s.updateBoard_detail(tempVO);
		} catch (RemoteException e2) {
			e2.printStackTrace();
		}
		ChangePane cp = Main_page_controller.fxmlLoad.getController();
		cp.clearing();
		ChangePane.changePane(getClass().getResource("share_detail.fxml"));
		cp.add();
	}
	...
	private Node createPage(int pageIndex) {
		...
		share_table.setItems(getTableViewData(from, to));
