...
import kr.or.ddit.rms.shelter.sharing_info_board.Share_detailSController;
...
public class Review_boardTbSController implements Initializable {
	...
	private void tableSet() {
		...
		try {
			lists = (ArrayList<Board_detailVO>) Main.rb_s.getBoard_detailAll(); // 목록을 전체 가져옴
			if (lists.isEmpty()) {
				return;
			}
	...
	private void findMember() {
		...
		if (Table_search_cb.getSelectionModel().isEmpty()) {
			...
			allBoardData = FXCollections.observableArrayList();
			try {
				lists = (ArrayList<Board_detailVO>) Main.rb_s.getSearchBoard_detail(vo);
				for (int i = 0; i < lists.size(); i++) {
					allBoardData.add(lists.get(i)); // 목록 전체 길이 구하기
				}
				tableSet();
			...
			case "제목":
				try {
					...
					ArrayList<Board_detailVO> searchList = (ArrayList<Board_detailVO>) Main.rb_s
	...
	private void gotoreviewAdd() {
		if (lists.isEmpty()) {
			Review_board_addSController.bc_num = "3";
		} else {
			try {
				lists = (ArrayList<Board_detailVO>) Main.rb_s.getBoard_detailAll(); // 목록을 전체 가져옴
				...
				Review_board_addSController.board_num = tempBns.get(0).getBc_idx();
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
			Review_board_addSController.bc_num = review_table.getItems().get(0).getBc_num();
		}
	...
	private void gotoreviewDetail() {
		Review_board_detailSController.vo = review_table.getSelectionModel().getSelectedItem();

		Board_detailVO tempVO = Review_board_detailSController.vo;
		int cntPlus = Integer.parseInt(tempVO.getBoard_cnt()) + 1;
		tempVO.setBoard_cnt(cntPlus + "");
		Review_board_detailSController.vo = tempVO;
		try {
			Main.rb_s.updateBoard_detail(tempVO);
