...
public class Free_board_detailController implements Initializable{
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		freeboard_id_tf.setDisable(true);
		freeboard_upd_btn.setOnAction(e->modalupd());
		report_btn.setOnAction(e->modalReport());
		
		System.out.println(Login_controller.log.getMem_id());
		freeboard_comment_name_textfield.setText(Login_controller.log.getMem_id());
		...
		freeboard_del_btn.setOnAction(e->{
			...
			if(delcheck) {
				try {
					Main.fb_a.deleteBoard_detail(Free_board_detailController.vo);
	...
	private void addCom() {
		if(freeboard_comment_add_ta.getText().length()==0||freeboard_comment_add_ta.getText().length()<5) {
			...
			try {
				Main.fb_a.updateBoard_detail(vo);
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
	...
	private void updCheck(JFXTextField title, JFXTextArea content, Stage writePage) {
		if(title.getText().isEmpty()) {
			...
			try {
				int cnt = Main.fb_a.updateBoard_detail(uvo);
				if(cnt>0) {
					alertInfo("수정 완료");
					vo=uvo;
					freeboard_title_tf.setText(vo.getTitle());
