...
public class Notice_User_Controller implements Initializable {
	...
	private void init() {
		allBoardData = FXCollections.observableArrayList();
	      try {
	         lists = (ArrayList<NoticeVO>) Main.n_u.getNoticeAll();
	...
	private void NoticeSearch() {
		...
		if (!search_txtF.getText().isEmpty()) {
			...
			allBoardData = FXCollections.observableArrayList();
			try {
				lists = (ArrayList<NoticeVO>) Main.n_u.getNoticeTextSearch(vo);
				for (int i = 0; i < lists.size(); i++) {
					allBoardData.add(lists.get(i));
				}
			} catch (RemoteException e) {
	...
	private void gotoNoticeDetail() {
		Notice_User_Detail_Controller.vo = notice_table.getSelectionModel().getSelectedItem();
