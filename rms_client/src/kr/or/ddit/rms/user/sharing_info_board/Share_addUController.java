...
public class Share_addUController implements Initializable {
	public static String bc_num;
	public static String board_num;
	public static String fileName;
	@FXML
	JFXTextField share_newtitle_tf;
	@FXML
	HTMLEditor share_content_ta;
	@FXML
	JFXTextField share_newfile_tf;
	@FXML
	JFXButton share_filefind_btn;
	@FXML
	JFXButton share_cancel_btn;
	@FXML
	JFXButton share_add_btn;
	...
	private void fileAdd() {
		...
		try {
			...
			File downloadFile = new File(
					"\\\\Sem-pc\\공유폴더\\Rescuedog\\Sharing_info\\" + bc_num + "," + board_num + fileName);
	...
	private void addCheck(JFXTextField share_newtitle_tf, HTMLEditor share_content_ta) {
		if (share_newtitle_tf.getText().isEmpty()) { // title 비어있을때
			...
			vo.setBc_num(Share_addUController.bc_num);
			vo.setBoard_img(Share_addUController.fileName);
			Board_classVO tempBn = new Board_classVO();
			tempBn.setBc_num(Share_addUController.bc_num);
			try {
			...
			vo.setCom_idx("1");
			try {
				Main.sib_u.insertBoard_detail(vo);
				ChangePane cp = Main_page_controller.fxmlLoad.getController();
				cp.clearing();
				ChangePane.changePane(getClass().getResource("share_table.fxml"));
				cp.add();
