...
public class Review_board_detailUController implements Initializable {
	public static Board_detailVO vo; // 목록화면에서 선택했을 때 선택한 값을 저장하기 위한 vo
	@FXML
	JFXTextField review_title_tf;
	@FXML
	JFXTextField review_id_tf;
	@FXML
	ImageView Layout_logo_note_img;
	@FXML
	ImageView review_content_iv;
	@FXML
	Label review_upd_date_label;
	@FXML
	Label review_cnt_label;
	@FXML
	WebView review_content_wv;
	@FXML
	JFXTextField review_comment_name_textfield;
	@FXML
	JFXTextArea review_comment_add_ta;
	@FXML
	JFXButton review_comment_add_btn;
	@FXML
	JFXButton review_upd_btn;
	@FXML
	JFXButton review_del_btn;
	@FXML
	JFXButton review_gotolist_btn;
	@FXML
	JFXButton report_btn;
	@FXML
	TableView<CommentsVO> comment_table;
	@FXML
	TableColumn<CommentsVO, String> comment_table_col1;
	@FXML
	TableColumn<CommentsVO, String> comment_table_col2;
	@FXML
	TableColumn<CommentsVO, String> comment_table_col3;
	@FXML
	TableColumn<CommentsVO, String> comment_table_col4;
	@FXML
	TableColumn<CommentsVO, String> comment_table_col5;

	ObservableList<CommentsVO> allComData;
	ArrayList<CommentsVO> comLists;
	@FXML
	AnchorPane forRemove_vb;

	@Override
	public void initialize(URL location, ResourceBundle resources) {
		Layout_logo_note_img.setOnMouseClicked(e -> sendNote());
		review_title_tf.setText(vo.getTitle());
		review_id_tf.setText(vo.getCustom_id());
		review_upd_date_label.setText(vo.getUpd_date());
		review_cnt_label.setText(vo.getBoard_cnt());
		contentLoad();
		review_title_tf.setEditable(false);
		review_id_tf.setEditable(false);
		review_content_wv.setDisable(true);
		review_upd_btn.setOnAction(e -> modalupd());
		report_btn.setOnAction(e -> modalReport());
		if (!vo.getCustom_id().equals(Login_controller.log_c.getCustom_id())) {
			review_upd_btn.setDisable(true);
			review_del_btn.setDisable(true);
		}
		review_comment_name_textfield.setText(Login_controller.log_c.getCustom_name());
		review_comment_name_textfield.setDisable(true);
		commentTableSet();
		imgSet();
		review_gotolist_btn.setOnAction(new EventHandler<ActionEvent>() {
		...
		review_del_btn.setOnAction(e -> {
			boolean delcheck = alertConfirm("삭제 하시겠습니까?");
			if (delcheck) {
				try {
					Main.rb_u.deleteBoard_detail(Review_board_detailUController.vo);
					ChangePane cp = Main_page_controller.fxmlLoad.getController();
					cp.clearing();
					ChangePane.changePane(getClass().getResource("review_board_table.fxml"));
					cp.add();
	...
	private void contentLoad() {
		WebEngine webEngine = review_content_wv.getEngine();
	...
	private void imgSet() {
		if (vo.getBoard_img() != null) {
			File loadFile = new File("\\\\Sem-pc\\공유폴더\\Rescuedog\\Review_board\\" + vo.getBc_num() + ","
			...
			try {
				...
				review_content_iv.setImage(img);
			} catch (IOException e) {
				forRemove_vb.getChildren().remove(review_content_iv);
	...
	private void commentTableSet() {
		...
		review_comment_add_btn.setOnAction(e -> addCom());
	...
	private void sendNote() {
		...
		send.setOnAction(e -> {
			if (note_send_to_txtfield.getText().isEmpty()) { // 받는 사람이 비어있을때
			...
			} else if (vo.getCustom_id().equals(Login_controller.log_c.getCustom_id())) {
				...
				vo1.setNote_id_from(Login_controller.log_c.getCustom_id()); // 받는 사람에 보내는 사람(로그인한) id 넣기
	...
	private void addCom() {
		if (review_comment_add_ta.getText().length() == 0 || review_comment_add_ta.getText().length() < 5) {
			...
			try {
				Main.rb_u.updateBoard_detail(vo);
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
			...
			temp.setBc_num(vo.getBc_num());
			temp.setBoard_num(vo.getBoard_num());
			temp.setComment_content(review_comment_add_ta.getText());
			temp.setCustom_id(Login_controller.log_c.getCustom_id());
			try {
				Main.cs.insertComments(temp);
				review_comment_add_ta.clear();
	...
	private void modalReport() {
		...
		idTo.setText(this.review_id_tf.getText());
		...
		ok.setOnAction(e -> {
			...
			if (check) {
				try {
					...
					bvo.setMem_id(this.review_id_tf.getText());
	...
	private void modalupd() {
		...
		Parent parent = null;
		try {
			parent = FXMLLoader.load(getClass().getResource("review_board_detail_upd.fxml"));
		} catch (IOException e) {
			e.printStackTrace();
		}

		JFXButton cancel = (JFXButton) parent.lookup("#review_cancel_btn");
		JFXButton ok = (JFXButton) parent.lookup("#review_ok_btn");
		JFXTextField title = (JFXTextField) parent.lookup("#review_title_tf");
		JFXTextField id = (JFXTextField) parent.lookup("#review_customId_tf");
		JFXTextArea content = (JFXTextArea) parent.lookup("#review_content_ta");
		title.setText(this.review_title_tf.getText());
		id.setText(this.review_id_tf.getText());
	...
	private void updCheck(JFXTextField title, JFXTextArea content, Stage writePage) {
		if (title.getText().isEmpty()) {
			...
			try {
				int cnt = Main.rb_u.updateBoard_detail(uvo);
				if (cnt > 0) {
					alertInfo("수정 완료");
					vo = uvo;
					review_title_tf.setText(vo.getTitle());
